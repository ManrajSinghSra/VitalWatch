...
const diseaseTerms = ["dengue", "influenza", "typhoid", "cholera", "malaria", "measles", "hepatitis", "diarrhea", "diarrhoea"];

function buildReportSignals(reports) {
  const counts = new Map();
  reports.forEach((report) => {
    const text = `${report.originalName || ""} ${report.source || ""} ${report.description || ""}`.toLowerCase();
  ...
  return [...counts.entries()].map(([name, count], index) => ({
    ...
    location: "Uploaded reports",
    cases: count,
    sev: index === 0 ? "high" : index === 1 ? "medium" : "low",
  }));
}
...
export default function LandingPage() {
  ...
  const reportSignals = useMemo(() => buildReportSignals(reports), [reports]);
  const heroStats = [
    { num: reports.length, label: "Reports Uploaded" },
    { num: new Set(reports.map((report) => report.source || "Other")).size, label: "Sources Connected" },
    { num: reports.filter((report) => report.status === "processed").length, label: "Processed Reports" },
  ];
  const diseaseChips = reportSignals.slice(0, 4).map((signal, index) => ({
    name: signal.name,
    cases: `${signal.cases} report${signal.cases === 1 ? "" : "s"}`,
    accent: [
    ...
    <div className="relative isolate min-h-screen overflow-x-hidden bg-slate-50 text-slate-800">
      ...
      <section className="relative z-10 mx-auto grid max-w-[1280px] gap-12 px-6 pb-20 pt-12 md:px-10 lg:grid-cols-[1.02fr_0.98fr] lg:items-start lg:pt-20">
        <div className="hero-fade-up relative z-10">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            {(diseaseChips.length ? diseaseChips : [{ name: "No report signals yet", cases: "Upload reports from admin", accent: "border-cyan-200 bg-cyan-50/80 text-cyan-700" }]).map((chip, index) => (
              <div
                key={chip.name}
      ...
      <section className="defer-section relative z-10 mx-auto max-w-[1280px] px-6 py-16 md:px-10">
        ...
        <div className="mx-auto grid max-w-5xl gap-5">
          {(reportSignals.length ? reportSignals : []).map((alert, index) => (
            <div
              key={alert.id}
        ...
        {!reportSignals.length && (
          <div className="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 text-center text-sm text-slate-500">
            No live disease signals yet. Upload reports from the admin panel to populate this section.
          </div>
        )}
      </section>
