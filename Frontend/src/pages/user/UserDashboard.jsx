...
export default function UserDashboard() {
  ...
  const heroStats = [
    ...
    { num: new Set(reports.map((report) => report.source || "Other")).size, label: "Sources" },
    { num: reports.filter((report) => report.status === "processed").length, label: "Processed" },
  ];
    ...
    <div className="relative isolate flex min-h-screen flex-col overflow-x-hidden bg-slate-50 text-slate-800">
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-[1280px] flex-1 gap-4 px-8 pb-12 xl:grid-cols-[240px_minmax(0,1fr)_300px] xl:grid-rows-[auto_auto]">
        ...
        <div className="w-full xl:col-span-2">
          ...
          {tab === "reports" && (
            <CardBox>
              <CardHeader title={`Uploaded Reports (${reports.length})`} />
                ...
                {reports.map((report) => (
                  <div key={report._id} className="flex items-center gap-4 border-b border-slate-200 px-4 py-4 last:border-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold text-slate-800">{report.originalName}</p>
                      <p className="text-xs text-slate-500">{report.source} · {new Date(report.createdAt).toLocaleDateString()}</p>
                    </div>
                    <Badge>{report.status}</Badge>
                  </div>
                ))}
                {!reports.length && <div className="p-5 text-sm text-slate-500">No reports uploaded yet.</div>}
              </div>
            </CardBox>
          )}
        ...
        <div className="xl:col-start-1 xl:row-start-2">
          <DataSourcesPanel reports={reports} />
        </div>
